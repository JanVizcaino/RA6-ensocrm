...
const CardSortingGame: React.FC = () => {
    ...
    // Refs para valores fiables en closures
    ...
    const generateNewCard = (): Card => {
        return CardSchema.parse({
            ...
            color: ColorEnum.options[
                Math.floor(Math.random() * ColorEnum.options.length)
            ],
            shape: ShapeEnum.options[
                Math.floor(Math.random() * ShapeEnum.options.length)
            ],
            number: Math.floor(Math.random() * 4) + 1,
            size: SizeEnum.options[
                Math.floor(Math.random() * SizeEnum.options.length)
            ],
        });
    };
    ...
    const changeRule = () => {
        ...
        setStats((prev) => ({
            ...prev,
            ruleChanges: prev.ruleChanges + 1,
            streak: 0,
        }));
    };
    ...
    const handleCardSelection = (refCard: Card) => {
        ...
        const isPerseverative =
            !isMatch &&
            !!previousRule &&
            checkMatch(targetCard, refCard, previousRule);

        setFeedback(isMatch ? "correct" : "incorrect");

        // Actualizar refs síncronamente
        attemptsRef.current += 1;
        if (isMatch) correctRef.current += 1;
        ...
        setStats((prev) => {
            ...
            return {
                ...
                perseverativeErrors:
                    prev.perseverativeErrors + (isPerseverative ? 1 : 0),
            };
        });
          ...
          `}
            ...
            <div className="absolute bottom-8 w-full text-center z-10 pointer-events-none">
                <p className="text-slate-400 text-sm font-medium bg-slate-900/50 inline-block px-4 py-2 rounded-full">
                    Haz click en una de las 4 figuras de abajo para clasificar
                    la de arriba
